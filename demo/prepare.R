...
# collect BRCA samples
code.brca <- as.character(tss$code[tss$study_name == "Breast invasive carcinoma"])
seg.brca <- do.call(rbind, lapply(code.brca,
  function(code) {
    seg[grep(paste0("TCGA-", code), seg$sample), ]
  }
))
...
samples.brca <- unique(seg.brca$sample)
...
write_seg(seg.brca, "tcga-brca.seg")
